...
Usage:
    python workflows/batch/batch_embeddings.py /path/to/by_species
    python workflows/batch/batch_embeddings.py /path/to/by_species --model resnet50
    python workflows/batch/batch_embeddings.py /path/to/by_species --cpu  # Force CPU mode
...
try:
    ...
    print("Install with: conda install faiss-cpu numpy tqdm")
    sys.exit(1)
...
try:
    ...
    print("Error: Cannot import from plantnet package.")
    print("Make sure plantnet is installed: pip install -e .")
    sys.exit(1)
...
def main():
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help=f"Output directory for embeddings database (default: {EMBEDDINGS_DIR})",
    )

    args = parser.parse_args()

    # Use default embeddings directory if not specified
    if args.output is None:
        args.output = EMBEDDINGS_DIR
