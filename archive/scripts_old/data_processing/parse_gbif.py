#!/usr/bin/env python3
"""
Script to parse GBIF export files (multimedia.txt and occurrence.txt) into interactive data structures.

This script loads the two tab-delimited GBIF files and provides various methods
to interact with and analyze the biodiversity observation data.
...
class GBIFParser:
    ...
    gbif_dir = "./plantnet_gbif"
    multimedia_file = "multimedia.txt"
    occurrence_file = "occurrence.txt"

    def __init__(self, gbif_dir=None):
        """
        Initialize the parser.

        Args:
            gbif_dir: Path to GBIF directory (default: ./plantnet_gbif)
        """
        self.gbif_dir = gbif_dir or self.gbif_dir
...
def main():
    ...
    print("\nSearching for 'Acacia' species:")
    acacia_obs = parser.search_by_species("Acacia")
    print(f"  Found {len(acacia_obs)} observations")
    if acacia_obs:
        print(f"  Example: {acacia_obs[0].get('scientificName')}")

    # Get complete observation
    if parser.get_occurrences():
        first_gbif_id = parser.get_occurrences()[0]["gbifID"]
        print(f"\nComplete observation for GBIF ID {first_gbif_id}:")
        complete = parser.get_complete_observation(first_gbif_id)
        print(f"  Species: {complete['occurrence'].get('scientificName')}")
        print(f"  Country: {complete['occurrence'].get('countryCode')}")
        print(f"  Number of images: {len(complete['multimedia'])}")

    print("\n" + "=" * 70)
    print("Use parser.get_multimedia() and parser.get_occurrences()")
    print("to access the data directly for custom analysis.")
    print("=" * 70)
